...
projection_30yravgs = read.csv(paste0('../processed/loca2_30yravgs_',domain,'.csv'))
projection_30yravgs$y <- projection_30yravgs$y+14
projection_30yravgs <- subset(projection_30yravgs,y>=2006)
period_count <- length(unique(projection_30yravgs$y))

# Load historical 
P_hist <- subset(projection_30yravgs,y==2006)$pr # units are mm
T_hist <- subset(projection_30yravgs,y==2006)$tavg
Model <- subset(projection_30yravgs,y==2006)$model
Variant <- subset(projection_30yravgs,y==2006)$variant
SSP <- subset(projection_30yravgs,y==2006)$ssp

# read in changes in P and T for each year (30 year climate windows)
GCM_data <- as.data.frame(cbind(P_hist,T_hist))
colnames(GCM_data) <- c("Pr_total", "Tas_mean")
GCM_data$D_pr <- 0
GCM_data$D_pr_lm <- 0
GCM_data$D_tas <- 0
GCM_data$Model <- Model
GCM_data$Variant <- Variant
GCM_data$SSP <- SSP
GCM_data$Year <- "Hist(1992-2021)"
GCM_data$period <- base_center

for (i in 1:(period_count-1)) {
  period <- paste0('Fut(',1992+i,'-',2021+i,')')
  
  Pr_total <- subset(projection_30yravgs,y==2006+i)$pr # units are mm
  Tas_mean <- subset(projection_30yravgs,y==2006+i)$tavg
  Model <- subset(projection_30yravgs,y==2006+i)$model
  Variant <- subset(projection_30yravgs,y==2006+i)$variant
  SSP <- subset(projection_30yravgs,y==2006+i)$ssp

  D_pr <- ((Pr_total-P_hist)/P_hist)*100
  D_tas <- Tas_mean-T_hist
  deltas <- as.data.frame(cbind(D_pr, D_tas))

  lm_pr_fut <- ((lm_fits$slope * (base_center+i-14)) + lm_fits$intercept)
  lm_fits$D_pr_lm <- ((lm_pr_fut - lm_pr_hist) / lm_pr_hist)*100

  deltas$Pr_total <- Pr_total 
  deltas$Tas_mean <- Tas_mean 
  deltas$Model <- Model
  deltas$Variant <- Variant
  deltas$SSP <- SSP
  deltas$Year <- period
