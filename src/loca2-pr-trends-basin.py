...
# %%
files = glob.glob('../data/loca2/loca2-projections-30y-basin/*_'+my_basin+'-*.csv')
...
for f in files:
    ...
    variant = f_name.split('_')[1]
    ssp = f_name.split('_')[2]

    df = pd.read_csv(f)
    df = df.groupby('Year (30y start)',as_index=False).aggregate({'Pr (mm)':np.sum,'Tave (degC)':np.mean})
...
for mvs in mvs_list:
    ...
    a, b = linear_regression(data.y, data['pr'], ylim=(850,1300),title=mvs,print=False)
...
# %%
fits.to_csv('../processed/loca2_lmfits_'+str(yr_start)+'_basin-'+my_basin+'.csv',index=False)

# %%
t_diffs = pd.DataFrame(projections.groupby(['mvs']).apply(lambda x: difference(x,'tavg',0)))
t_diffs.columns = np.arange(yr_start,2072)
t_diffs = t_diffs.melt(ignore_index=False).reset_index()
t_diffs.columns = ['mvs','y','tavg_diff']
projections = pd.merge(projections,t_diffs,on=['mvs','y'])

# %% 
for t in [1,2,3,4,5]:
    diff_levels = projections.groupby(['mvs']).apply(lambda x: x['y'].values[np.searchsorted(x['tavg_diff'],t)-1]+window-1)
    if t==1:
...
# %%
warming_levels.to_csv('../processed/loca2_warming_levels_basin-'+my_basin+'.csv',index=False)
